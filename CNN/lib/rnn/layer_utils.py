...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        h_t = np.dot(prev_h, self.params[self.wh_name]) + np.dot(x, self.params[self.wx_name])
        next_h = np.tanh(h_t + self.params[self.b_name])
        meta = {'x': x, 'next_h': next_h, 'prev_h': prev_h}
    ...
    def step_backward(self, dnext_h, meta):
        ...
        temp = (1 - np.square(meta['next_h'])) * dnext_h
        dx = np.dot(temp, self.params[self.wx_name].T)
        dprev_h = np.dot(temp, self.params[self.wh_name].T)
        dWx = np.dot(meta['x'].T, temp)
        dWh = np.dot(meta['prev_h'].T, temp)
    ...
    def forward(self, x, h0):
        ...
        h0, meta = h0, 0
        for i in range(T):
            h0, meta = self.step_forward(x[:,i,:], h0)
            h[:,i,:] = h0
    ...
    def backward(self, dh):
        ...
        for i in reversed(range(T)):
            dx[:,i,:], dh0, dWx, dWh, db = self.step_backward(dh0+dh[:,i,:], self.meta[i])
...
class LSTM(object):
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        dc = dnext_h * (1-np.tanh(next_c)**2) * o + dnext_c
        dprev_c = f * dc
        di = i * (1-i) * g * dc
        df = f * (1-f) * prev_c * dc
        do = dnext_h * o * (1-o) * np.tanh(next_c)
        dg = (1-g**2) * i * dc
        temp = np.hstack((di,df,do,dg))
        dWx = np.dot(x.T, temp)
    ...
    def forward(self, x, h0):
        ...
        for i in range(T):
            x_t = x[:,i,:]
            h0, prev_c, meta = self.step_forward(x_t, h0, prev_c)
            h[:,i,:] = h0
            self.meta.append(meta)
    ...
    def backward(self, dh):
        ...
        for i in reversed(range(T)):
            dxt, dprev_h, dprev_c, dWx, dWh, db = self.step_backward(dprev_h+dh[:,i,:], dprev_c, self.meta[i])
            self.grads[self.wx_name] += dWx
            self.grads[self.wh_name] += dWh
            self.grads[self.b_name] += db
            dx[:,i,:] = dxt
...
class word_embedding(object):
    ...
    def forward(self, x):
        ...
        N, T = x.shape
        out = np.zeros((N, T ,self.vec_dim))
        for i in range(N):
            for j in range(T):
                out[i,j,:] = self.params[self.w_name][x[i,j]]
        self.meta = x
